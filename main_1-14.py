...
def initHiveTables():
    ...
    initUpdate_result_table(host="localhost", database="metro_hangzhou", TableName="inputStation_Nums",
                            sql="select stationID,count(*) as nums from record1_14  where status='1'  group by stationID  ORDER BY nums DESC")
    # 导出到Mysql
    #  2.2 MySQL操作
...
def updateHiveTables():
    ...
    overwriteUpdate_data_table(host="localhost", database="metro_hangzhou",
                               localDataPath="/root/Metro_Hangzhou/Metro_train_m/record_01-14.csv", dataTableName="record1_14")
    ...
    #                              sql="select count(DISTINCT stationID ) from record1_14")
    #  # （2）总设备数
    # singleInsert_result_table(host="localhost", database="metro_hangzhou", TableName="four_total_Nums",
    #                              sql="select count(DISTINCT deviceID) from record1_14")
    #  # （3）总用户量
    # singleInsert_result_table(host="localhost", database="metro_hangzhou", TableName="four_total_Nums",
    #                              sql="select count(DISTINCT userID) from record1_14")
    #  # （4）总人次
    # singleInsert_result_table(host="localhost", database="metro_hangzhou", TableName="four_total_Nums",
    #                              sql="select count(DISTINCT time) from record1_14")
    ...
    overwriteUpdate_result_table(host="localhost", database="metro_hangzhou", TableName="outputStation_Nums",
                            sql="select stationID,count(*) as nums from record1_14  where status='0'  group by stationID  ORDER BY nums DESC")
    # 导出到Mysql
    #  3.2 MySQL操作
    create_mysql_table(host='localhost', user='hadoop', passwd='hadoop', port=3306, db='metro_hangzhou',
    ...
    overwriteUpdate_result_table(host="localhost", database="metro_hangzhou", TableName="lineID_Nums",
                            sql="select lineID,count(*) from record1_14 group by lineID ORDER BY lineID")
    # 导出到Mysql
    #  4.2 MySQL操作
    create_mysql_table(host='localhost', user='hadoop', passwd='hadoop', port=3306, db='metro_hangzhou',
